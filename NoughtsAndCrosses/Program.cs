...
namespace NoughtsAndCrosses
{
    class Program
    {
      ...
      //  static Player[] players = new Player[2];
                ...
                if (theGame.GameActive)
                {
                    for (int verticalCount = 0; verticalCount < gameSize; verticalCount++)
                    {
                        for (int horizontalCount = 0; horizontalCount < gameSize; horizontalCount++)
                        {
                            ...
                            Console.Write(theGame.CurrentNode.Theboard.getSquareID((SquareID)(verticalCount * 3 + horizontalCount)));
                    ...
                    Console.Write("It is " + theGame.Activeplayer.Name + "'s go");
                    Console.SetCursorPosition((2 * gameBoardMargin) + (2 * gameSize - 1) + 1, 13);
                    Console.Write("Place your " + theGame.Activeplayer.Marker);
                }
        ...
        static private void drawBox(int left, int top, int width, int height, string message, ConsoleColor back, ConsoleColor margin, ConsoleColor text)
        {
            string topBottom = "";
            string middle = "";
            Console.BackgroundColor = back;
            Console.ForegroundColor = margin;
            for (int lineCount = 0; lineCount < width; lineCount++)
            {
                topBottom += "*";
                if(lineCount == 0 || lineCount == width - 1)
                {
                    middle += "*";
                }
                else
            ...
            {
                Console.SetCursorPosition(left, top + rowCount);
            ...
            Console.SetCursorPosition(left+1, top + Convert.ToInt32(height / 2));
        ...
        static void Main(string[] args)
        {
          ...
          /*  string newNet = " ";
            while (newNet != "")
            {
                foreach (string name in con.GetNetNames)
                {
                    Console.WriteLine(name);

                }
                Console.WriteLine("Enter Net name to create or null to continue");
                newNet = Console.ReadLine();
                if(newNet != "")
                {
                    con.CreateNet(newNet);
                }
            }*/
            ...
            while (programOn)
            {
                ...
                switch (mainMenuOption)
                {
                    ...
                    case MainMenuChoice.START_NEW_GAME:
                        //drawBox(10, 10, 12, 7, "Hello Jacob", ConsoleColor.Blue, ConsoleColor.Yellow, ConsoleColor.DarkYellow);
                        //Console.In.ReadLine();
                        InitialiseGame();
                        gamePlayed = true;
                        //gameOn = true;
                ...
                while (theGame.GameActive)
                {
                    if (theGame.Activeplayer.AI)
                    {
                        //theGame.AIMove();
                        DrawBoard();
                    }
                    else
                    {
                        ...
                        switch (currentCommand)
                        {
                            ...
                            case 'q':
                                // theNet.GameActive = false;
                                break;

                            case '\r':
                                if (theGame.CurrentNode.Theboard.BoardData[currentX + 3 * currentY] == SquareState.Blank)
                                {
                                    theGame.PlayerMove((SquareID)(currentX + 3 * currentY));
                                    /*  if (activePlayer == 1)
                                      {
                                          activePlayer = 0;
                                      }
                    ...
                    if (theGame.GameWon)
                    {
                        drawBox(10, 3, 30, 5, "Game Over, " + theGame.Activeplayer.Name + " won", ConsoleColor.Blue, ConsoleColor.Yellow, ConsoleColor.DarkYellow);
                    }
                    else if(!theGame.GameActive)
                    {
                        drawBox(10, 3, 30, 5, "Game Over, it's a draw", ConsoleColor.Blue, ConsoleColor.Yellow, ConsoleColor.DarkYellow);
                    }
                }
 ...
 /*           ClearTextWindow();
       ...
       /*     Console.SetCursorPosition((2 * gameBoardMargin) + (2 * gameSize - 1) + 1, 5);
        ...
        private static void InitialSetup()
        {
            /*  players[0] = new Player();
              ...
              players[0].Marker = SquareState.Cross;
              players[1].Name = "Player 2";
              players[1].Marker = SquareState.Nought;
              activePlayer = 0;*/
          //  theNet = new NeuralNet("Basic");
        }
      ...
      /*  private static void LoadGame()
        ...
        private static void InitialiseGame()
        {
            ...
            NeuralNet theNet = con.getNet("default");
            theGame = new Game(theNet, true);
            theGame.StartGame();
